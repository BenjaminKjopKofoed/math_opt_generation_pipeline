...
from challenge.prompts import (
    get_code_prompt,
    get_code_prompt_similarity,
    get_code_prompt_static_example,
)
from challenge.solution_manager import (
    get_best_solution_as_result,
    get_solution_count_dict,
)
...
def get_prompt(item: dict, text_similarity: TextSimilarity) -> (PromptTemplate, dict):
    ...
    if text_similarity:
        most_similar_problem, similarity = text_similarity.get_most_similar_problem(
            item
        )
...
def process_item(item: dict, text_similarity: TextSimilarity) -> (dict, dict):
    ...
    for temperature in params["temperatures"]:
        for it in range(1 + math.floor(temperature * params["temp-coeff"])):
            ...
            try:
                ...
                print(
                    f"\tRun with temp. {temperature}, iteration {it} discarded due to errors: ",
                    e,
                )
    ...
    best_result_majority, best_result_llm = get_best_solution_as_result(
        solution_freq, result_keys, item["question"]
    )
...
def process_data(test_data: list[dict], args: dict) -> (list[dict], list[dict]):
    ...
    if params["similarity"]:
        train_data = read_json(params["data/input/task3_train_public.json"])
